...
import static com.dtbcds.stacktonearbychests.StackToNearbyChests.currentStackToNearbyContainersButton;
...
public class InventoryActions {
    ...
    public static void init() {
        // No-op, registered via mixin calls
    }

    public static InteractionResult onBeforeClickSlot(int slotId, int button, ClickType actionType,
            LocalPlayer player) {
    ...
    public static void stackToNearbyContainers() {
        forEachContainer(InventoryActions::quickStack, ModOptions.get().behavior.stackingTargets,
                ModOptions.get().behavior.stackingTargetEntities);
    }

    public static void stackToNearbyContainers(Item item) {
        forEachContainer(screenHandler -> quickStack(screenHandler, item), ModOptions.get().behavior.stackingTargets,
                ModOptions.get().behavior.stackingTargetEntities);
    }

    public static void restockFromNearbyContainers() {
        forEachContainer(InventoryActions::restock, ModOptions.get().behavior.restockingSources,
                ModOptions.get().behavior.restockingSourceEntities);
    }

    public static boolean canMerge(ItemStack stack, ItemStack otherStack) {
        return stack.getCount() < stack.getMaxStackSize() && ItemStack.isSameItemSameTags(stack, otherStack);
    }

    public static void forEachContainer(Consumer<AbstractContainerMenu> action, Collection<String> blockFilter,
        ...
        if (cameraEntity == null || world == null || interactionManager == null || player == null) {
            LOGGER.info("cameraEntity: {}, word: {}, interactionManager: {}, player: {}", cameraEntity, world,
        ...
        var task = new ForEachBlockContainerTask(client, cameraEntity, world, player, interactionManager, action,
                blockFilter);

        if (ModOptions.get().behavior.supportForContainerEntities.booleanValue() && !player.isPassenger()) {
            task.thenStart(new ForEachEntityContainerTask(client, player, action, cameraEntity, world,
                    interactionManager, entityFilter));
        }
    ...
    public static void quickStack(AbstractContainerMenu screenHandler) {
        ...
        Set<Item> itemsInContainer = slots.containerSlots().stream()
                ...
                .filter(item -> !ModOptions.get().behavior.itemsThatWillNotBeStacked
                        .contains(ForgeRegistries.ITEMS.getKey(item).toString()))
                .collect(toSet());
    ...
    private static void moveAll(AbstractContainerMenu screenHandler, List<Slot> playerSlots, Set<Item> itemsToBeMoved) {
        playerSlots.stream()
                .filter(slot -> !(ModOptions.get().behavior.doNotQuickStackItemsFromTheHotbar.booleanValue()
                        && Inventory.isHotbarSlot(slot.getSlotIndex())))
                .filter(slot -> !InventoryActions.isSlotLocked(slot))
    ...
    public static void restock(AbstractContainerMenu screenHandler) {
        ...
        slots.playerSlots().stream()
                ...
                .filter(slot -> !ModOptions.get().behavior.itemsThatWillNotBeRestocked
                        .contains(ForgeRegistries.ITEMS
                                .getKey(slot.getItem().getItem())
                                .toString()))
                .forEach(slot -> slots.containerSlots().stream()
                        .filter(containerSlot -> ItemStack.isSameItemSameTags(slot.getItem(), containerSlot.getItem()))
                        .peek(containerSlot -> {
    ...
    public static void quickMove(AbstractContainerMenu screenHandler, Slot slot) {
        Minecraft client = Minecraft.getInstance();
        client.gameMode.handleInventoryMouseClick(screenHandler.containerId, slot.index, GLFW.GLFW_MOUSE_BUTTON_LEFT,
    ...
    private static boolean isSlotLocked(Slot slot) {
        if (StackToNearbyChests.IS_IPN_MOD_LOADED) {
            try {
                ...
                StackToNearbyChests.LOGGER
                        .warn("An exception occurred when determining whether the slot is locked by IPN mod", e);
            }
        }
